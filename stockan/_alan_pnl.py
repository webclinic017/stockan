...
def cumsum_clip(a,xfl=-99,xcap=99):
	'''
	calc cumsum of array a[] clip to [xfl,xcap] 
	'''
	b=[]
	tx=0
	for x in a:
...
def calc_ohlc_pnl(data,ret,prd=125,xfl=-1,xcap=1,debugTF=False):
	""" PnL calc period (days)
	"""
	smpl=data[['price','pbdate']].copy()
	ns=len(smpl)
	smpl.index=range(ns)
	#smpl.is_copy = False
	tablename="ohlc_pnl"
	...
	for j in range(4) :
		...
		jx=j*2
		if debugTF:
			print >> sys.stderr, "calc_ohlc_pnl():",j,ret[jx]['ticker'],ret[jx]['name']
		smpl.loc[:,'ticker']=ret[jx]['ticker']
		smpl.loc[:,'name']=ret[jx]['name']
		nsig[0]=len(ret[jx]["winsig"])
		nsig[1]=len(ret[jx+1]["winsig"])
		if nsig[0]>0:
			smpl.loc[ret[jx]["winsig"],'amount']=2
		if nsig[1]>0:
			smpl.loc[ret[jx+1]["winsig"],'amount']=-2
		smpl.loc[:,'cost']= smpl['price'].values
		mx=smpl.tail(prd).reset_index(drop=True)
		nm=len(mx)
		mx['ttr_hold']=map(lambda x:x/mx['price'][0]*100.-100,mx['price'].values)
		for k,(lx,ly) in enumerate(lsc.iteritems()) :
			if debugTF:
				print >> sys.stderr, "\t{}. lsc:{}".format(k,(lx,ly))
			tc=sum(map(abs,filter(lambda x: x/2 in ly,mx.amount)))
			if tc<1:
				print >> sys.stderr, "\tPnL skip {}".format(lx)
				continue
			(xf,xc)=ly
			#xtc=mx.amount.abs().sum()/2
			if debugTF:
				print >> sys.stderr,"\tPnL calc {} {} {} total:{}".format(lx,xf,xc,tc)
			df=smpl_pnl_calc(mx,xf,xc)
			df['lsc']=lx
			if lx == 'short':
				df['name']=ret[jx+1]['name']
			df['rrt']=df['pnls']/df['cost']*100.
			df['trr']=df['rrt'].cumsum()
			df['sig']= map(abs,map(lambda x: x/2 if x/2 in ly else 0 ,mx.amount))
			df['tsig']= df['sig'].cumsum()
			dm=pd.concat([dm,df])
	"""
